...
function BanquePage() {
  ...
  const handleValiderReleve=async()=>{
    ...
    try{
      ...
      for(const tx of txExtraites){
        ...
        // Rapprochement factures — utiliser facture_id IA ou chercher par montant+nom
        let facId = tx.facture_id;
        
        // Si pas de match IA, tenter matching TypeScript par montant exact
        if(!facId && isCr){
        ...
        if(facId){
          const isClientFac=facturesClient.some((f:any)=>f.id===facId);
          const isFournFac=facturesFourn.some((f:any)=>f.id===facId);
          if(isClientFac&&isCr) fcPay.push(facId);
          else if(isFournFac&&!isCr) ffPay.push(facId);
        }
      }
      ...
      for(const fid of fcPay){
        ...
        // Pour facture acompte: si ancienPaye = txMontant (même montant),
        // c'est le premier encaissement → ne pas additionner (éviter double comptage)
        // Pour les paiements suivants: additionner normalement
        const montantPaye = Math.abs(ancienPaye - txMontant) < 1 && fac?.type_facture==="acompte"
          ? txMontant  // premier encaissement acompte: montant_paye = tx seul
          : Math.round((txMontant + ancienPaye)*100)/100; // autres cas: cumuler
      ...
      setScanStep("done");
      load();
      // Charger les transactions du compte scanné ET sélectionner ce compte
      if(releveCompteId){
        setSelectedId(releveCompteId);
        loadTx(releveCompteId);
      } else if(selectedId){
        loadTx(selectedId);
      }
  ...
  const genererEDI=async()=>{
    ...
    const COMPTES_TVA_DEDUCTIBLE=["4411","6122","6125","6131","6132","6141","6145","6146","6347"];
    ...
    txEligibles.forEach((tx,i)=>{
      ...
      // Récupérer données complètes depuis facture matchée ou recherche par nom
      const facFourn=tx.facture_id?(facturesFourn as any[]).find((f:any)=>f.id===tx.facture_id):null;
      const facClient=tx.facture_id?(facturesClient as any[]).find((f:any)=>f.id===tx.facture_id):null;
      
      // Si facture fournisseur matchée → chercher le fournisseur par fournisseur_id ou nom
      const fourn = facFourn?.fournisseur_id
        ?(fournisseurs as any[]).find((f:any)=>f.id===facFourn.fournisseur_id)
        : facFourn?.fournisseur_nom
          ?(fournisseurs as any[]).find((f:any)=>f.nom===facFourn.fournisseur_nom)
          : tx.tiers_nom
            ?(fournisseurs as any[]).find((f:any)=>f.nom&&f.nom.toUpperCase().includes((tx.tiers_nom||"").toUpperCase().slice(0,4)))
            :null;

      // Si facture client matchée → chercher le client
      const client = facClient?.client_id
        ?(clients as any[]).find((f:any)=>f.id===facClient.client_id)
        : null;

      const tiers = fourn || client;
      ...
      const dateFacExcel=facFourn
        ...
        :facClient
          ?toExcelDate((facClient.date_facture||"").split("-").reverse().join("/"))
          :datePaieExcel;

      // Numéro de facture: depuis la facture matchée ou référence manuelle
      const factNum = facFourn?.numero || facClient?.numero || tx.reference_facture || `TX-${i+1}`;
      // Nom tiers: depuis la facture matchée en priorité
      const tiersNom = facFourn?.fournisseur_nom || facClient?.clients?.nom || tx.tiers_nom || tiers?.nom || "";
  ...
  const genererBilan=()=>{
    ...
    for(const tx of txExtraites){
      ...
      const applyTva=!isCr&&pcm.tva>0&&tx.categorie!=="encaissement_client";
      const ht=applyTva?Math.round(tx.montant/(1+pcm.tva/100)*100)/100:tx.montant;
      const tva=applyTva?Math.round((tx.montant-ht)*100)/100:0;
      // Écriture 5141
      rows.push([tx.date_operation,"BQ","5141",tx.libelle,isCr?String(tx.montant):"",isCr?"":String(tx.montant),tx.categorie,tx.reference_facture||""]);
      // Écriture compte charge/produit
      if(applyTva){
        rows.push([tx.date_operation,"BQ",tx.compte_comptable,tx.libelle,String(ht),"",tx.categorie,tx.reference_facture||""]);
        rows.push([tx.date_operation,"BQ","34552",`TVA ${tx.libelle.slice(0,40)}`,String(tva),"",tx.categorie,tx.reference_facture||""]);
      } else {
        rows.push([tx.date_operation,"BQ",tx.compte_comptable,tx.libelle,isCr?"":String(ht),isCr?String(ht):"",tx.categorie,tx.reference_facture||""]);
      }
    }
