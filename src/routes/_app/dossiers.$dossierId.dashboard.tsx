  ...
  useEffect(() => {
    (async () => {
      const [{ data: d }, { data: f }, { data: ffData }, { data: al }] = await Promise.all([
        ...
        // Ajouter montant_paye et montant_restant pour calculs corrects
        supabase.from("factures").select("statut,statut_paiement,montant_ht,montant_ttc,montant_tva,montant_paye,montant_restant,type_facture,date_facture,date_echeance").eq("dossier_id", dossierId),
        supabase.from("factures_fournisseurs").select("statut_paiement,montant_ttc,montant_paye,montant_restant").eq("dossier_id", dossierId),
