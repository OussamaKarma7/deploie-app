...
${JSON.stringify(data.factures_client.map((f: any) => ({ id: f.id, num: f.numero, client: f.clients?.nom, ttc: Number(f.montant_ttc), restant: Number(f.montant_restant||f.montant_ttc), paye: Number(f.montant_paye||0), mode: f.mode_reglement, echeance: f.date_echeance })))}

FACTURES FOURNISSEURS NON PAYÉES:
${JSON.stringify(data.factures_fourn.map((f: any) => ({ id: f.id, num: f.numero, fournisseur: f.fournisseur_nom, ttc: Number(f.montant_ttc), restant: Number(f.montant_restant||f.montant_ttc), paye: Number(f.montant_paye||0), mode: f.mode_reglement, echeance: f.date_echeance })))}
